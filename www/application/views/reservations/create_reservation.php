<div class="container mt-5">
    <h1>Welcome!
    <small class="text-muted">(choose what you want to reserve)</small>
    </h1>
    <div class="row my-3">
        <div class="col-5 text-center border border-info rounded my-3 mx-2 p-4">
            <h3 class="mb-3">Conference Rooms</h3>
            <a href="/reservations/meeting/create_by_date" class="btn btn-block btn-outline-info btn-options">Search by Free Time</a>
            <a href="/reservations/meeting/create_by_room" class="btn btn-block btn-outline-info btn-options">Search by Room</a>
        </div>
        <div class="col-5 text-center border border-info rounded my-3 mx-2 p-4">
            <h3 class="mb-3">Equipment</h3>
            <a href="/reservations/equipment/create_by_date" class="btn btn-block btn-outline-info btn-options">Search Equipment by Free Time</a>
            <a href="/reservations/equipment/create_by_item" class="btn btn-block btn-outline-info btn-options">Search by Specific Item</a>
        </div>
    </div>
    <div id="show"></div>
    <script src="/js/reservations.js"></script>
</div>
