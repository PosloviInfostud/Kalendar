<div class="max-w-md mx-auto">
    ...
    <div class="bg-white border-x sm:border-y sm:rounded shadow p-4 sm:p-8 mt-4">
        ...
        <h2 class="pl-2 font-normal text-lg xs:text-xl sm:text-2xl border-l-4 border-primary">Current members</h2>
                        ...
                        <tr>
                            ...
                            <!-- Check who is creator of the reservation and unabling his update or deletion -->
                                <?php if($meeting['creator_id'] != $member['user_id']) { ?>
                                    ...
                                    <td class="px-2 py-2">
                                        <button class="role_edit hover:bg-primary text-grey text-xs hover:text-white mr-2 py-1 px-2 border-2 hover:border-primary rounded focus:outline-none" data-roleid="<?= $member['res_role_id'] ?>" data-rolename="<?= $member['role'] ?>" data-name="<?= $member['name'] ?>" data-res="<?= $meeting['id'] ?>" data-user="<?= $member['user_id'] ?>"  data-creator="<?= $meeting['creator_id'] ?>">
                                            e
                                        </button>
                                        <button class="member_delete hover:bg-red text-grey text-xs hover:text-white mr-3 py-1 px-2 border-2 hover:border-red rounded focus:outline-none" data-res="<?= $meeting['id'] ?>" data-user="<?= $member['user_id'] ?>" data-creator="<?= $meeting['creator_id'] ?>">
                                            x
                                        </button>
                                    </td>
                                <?php }  else { ?>
        ...
        <?php if (in_array($user_id, $editors) && $meeting['status'] != 'expired') {  ?>
            <button class="w-full bg-primary-light hover:bg-primary text-white font-bold mt-4 py-2 px-4 rounded focus:outline-none" id="btn_add_new_member" data-toggle="modal" data-target="#addNewMember" data-res="<?= $meeting['id'] ?>">Add new member</button> 
        <?php } ?>       
    </div>
    <!-- Recurring events -->
    <?php if($meeting['recurring'] == 1) { ?>
    <div class="bg-white border-x sm:border-y sm:rounded shadow p-4 sm:p-8 mt-4">
        <h2 class="pl-2 font-normal text-lg xs:text-xl sm:text-2xl border-l-4 border-primary">Recurring Events</h2>
        <table class="w-full text-grey-darker text-sm mt-6 overflow-auto">
            <thead class="text-left bg-grey-light font-medium uppercase text-sm text-grey-dark border border-grey-light">
                <tr>
                    <th class="py-2 px-4 text-center"colspan=2>Upcoming dates</th>
                </tr>
            </thead>
            <tbody class="bg-grey-lighter">
                <?php foreach($child_dates as $date) { ?>
                    <tr>
                        <td class="md:w-1/2 p-2 text-center"><?= $date['start_time'] ?></td>
                        <td class="md:w-1/2 p-2 text-center">
                        <a class="no-underline" href="/reservations/meetings/<?= $date['id'] ?>">
                            <button class="hover:bg-primary text-grey text-xs hover:text-white mr-3 py-1 px-2 border-2 hover:border-primary rounded focus:outline-none">
                                v
                            </button>
                        </a>
                        ...
                        <a class="no-underline" href="/reservations/meetings/delete/<?= $date['id'] ?>">
                            <button class="hover:bg-red text-grey text-xs hover:text-white mr-3 py-1 px-2 border-2 hover:border-red rounded focus:outline-none">
                                x
                            </button>
                        </a>
                        </td>
...
<div class="hidden modal" id="editUserRoleModal">
    <div class="fixed pin z-50 overflow-auto bg-smoke-light flex">
        <div id="modal-content" class="relative p-8 md:p-12 bg-white sm:w-2/3 md:w-1/3 max-w-smd m-auto flex-col flex rounded shadow">
