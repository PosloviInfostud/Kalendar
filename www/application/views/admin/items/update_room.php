<form id="update_room_form" class="mt-8">
    ...
    <div class="mt-2 mb-8">
        <label for="capacity" class="font-light text-lg mb-2">Capacity</label>
        <input type="number" class="bg-grey-lighter font-light ml-4 p-2 w-auto border rounded" name="name" id="update_room_capacity" value="<?= $room['capacity'] ?>" min="4" max="50">
    </div>
    <div class="mt-2 mb-8">
        <label for="color" class="font-light text-lg mb-2">Color for Calendar <br><small class="text-grey text-sm">(required)</small></label>
        <input type="color" class="bg-grey-lighter font-light ml-4 mt-2 p-1 w-1/4 h-10 border rounded" id="update_room_color" value="<?= $room['color'] ?>">
    </div>
    <hr>
